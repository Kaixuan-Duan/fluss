...
public class TabletServerMetadataCache implements ServerMetadataCache {
    ...
    public Errors validateBucketCount(
        ...
        if (!initialMetadataApplied) {
            // The first metadata update has not been applied yet, so a missing table entry
            // means "not loaded yet" rather than "does not exist". Ask the client to refresh
            // metadata and retry instead of validating against the empty startup snapshot.
