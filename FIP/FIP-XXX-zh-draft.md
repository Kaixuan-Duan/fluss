...
仅当服务器实际知道目标的存储桶计数时，才会验证请求。当快照缺少表的存储桶布局纪元、分区的显式存储桶计数或非分区表的显式存储桶计数时，服务器返回 TABLET\_METADATA\_NOT\_READY 而不是允许请求通过。该错误告诉客户端服务器尚未收到目标元数据，无法确认或反驳桶计数。客户端应该重试相同的请求，而不刷新其元数据或重建存储桶分配器，因为服务器可能根本还没有跟上 - 客户端的存储桶计数可能是正确的。

这与 STALE\_METADATA 不同，STALE\_METADATA 意味着服务器已确认客户端的存储桶计数与实际不匹配。 STALE\_METADATA 触发元数据失效和桶分配器重建； TABLET\_METADATA\_NOT\_READY 仅触发重试。

当 TabletServer 刚刚启动且尚未收到任何元数据时，同样适用：快照为空，表的纪元不存在，并且服务器为每个携带存储桶计数的请求返回 TABLET\_METADATA\_NOT\_READY。纪元 0 的旧客户端（请求中没有存储桶计数）仍然允许通过，因为没有任何内容需要验证，并且所有分区的表级计数都是正确的。
