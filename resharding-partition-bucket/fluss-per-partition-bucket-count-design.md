...
**Paimon 在 Fixed Bucket 下如何 rescale**：必须先 `ALTER TABLE` 改桶数、再 `INSERT OVERWRITE` 重组数据；重组完成之前直接写旧分区会报错，而读不受影响。

对照到 Fluss 的四种表/分区组合，`ALTER TABLE` 之后的读写行为如下：

| 组合 | ALTER TABLE 后写已有分区/表 | ALTER TABLE 后写全新分区 |
| --- | --- | --- |
| 非分区 pk 表 | 报错，无法写入 | N/A |
| 非分区 log 表 | 存在`bucket.key`时会报错，无法写入 | N/A |
| 分区 pk 表 | 报错，无法写入 | 可以写入，按新 bucket 数 |
| 分区 log 表 | 存在`bucket.key`时会报错，无法写入 | 可以写入，按新 bucket 数 |
...
| --- | --- | --- | --- |
| Hash | 指定 `bucket.key` | **Fixed Bucket**：`BUCKET=numBuckets`、`BUCKET_KEY=bucketKeys` | 必须 `ALTER TABLE` 改桶数 + `INSERT OVERWRITE` 重组数据；重组前直接写旧分区会报错。读不受影响。 |
...
写新分区没有问题，真正的冲突点在于写旧分区/旧表。因在 Paimon 的 Fixed Bucket 模式下无法对未重刷数据的旧分区写入，所以 Fluss 旧分区可以写入，但 Tiering Service 立刻无法写入 Paimon。

**解决方案讨论**（针对 Fixed Bucket 冲突场景）：

1.  Fluss 也不接受写，和 Paimon 对齐。

2.  Fluss 接受写，但是 alter 后往旧分区写入的数据有丢失风险，ttl 之后分区被清理。

    *   a. 数据不能删除，让 ttl 失效。

    *   b. 二期做完，Fluss 和 Paimon 一起重刷旧数据（一期没实现）。
...
解决：考虑把这个读写元数据的操作，纳入coordinator 的单线程事件队列串行处理
...
> 本章范围：`table.datalake.enabled=false` 的分区表（湖流一体表见后续独立章节）。
...
4. **校验**：解除禁用后，ALTER 路径需自行校验合法性——湖表（`table.datalake.enabled=true`）拒绝（改桶数需 Rebuild-and-Swap，见 §2，本期不支持）、非分区表拒绝（需重建 assignment 并在 TabletServer 初始化 LogTablet，本期不支持）、新值须落在 `[1, MAX_BUCKET_NUM]`（下界 1、上界默认 4096），越界分别抛 `InvalidAlterTableException` 与 `TooManyBucketsException`。
...
**当前实现状态（待改造）**：HEAD 的 `backfillPartitionBucketCount` 仍是循环内对每个分区各调一次 `updatePartitionRegistration`（独立 `setData`），之后再单独 `updateTable`，为分步多次写、非单一事务。需改造为：回填遍历时只收集 op 不提交，连同表级更新 op 一起放入同一事务一次提交。

**6. 与并发建分区的安全性（竞态一）**

回填与表级切换运行在处理 ALTER 请求的线程上，而建分区（手动 DDL、写入侧动态补建走 RPC 线程，自动分区走 `AutoPartitionManager` 线程）不经同一串行队列，可与 ALTER 并发（§3.1 竞态一）。该竞态在本设计下无害，无需加锁。
...
因此竞态一的两种落点都合法，读写不会因此错桶。将读写元数据强行纳入 Coordinator 单线程事件队列可消除该竞态，但为一个无害竞态而串行化全部建分区/ALTER 得不偿失，本期不做。
...
**前提条件（已满足）**：`ClientRpcMessageUtils.toPartitionInfos` 已将 `PbPartitionInfo.bucket_count` 透传为 `PartitionInfo.getBucketCount()`（`hasBucketCount() ? getBucketCount() : null`，缺失映射为 `null` 而非 0）；`listPartitionInfos` 因此能把 per-partition 桶数带到读端。

**改造细节**：

1. **枚举器内部 `Partition` 携带桶数**：`FlinkSourceEnumerator.Partition` 新增 `@Nullable Integer bucketCount` 字段。`null` 表示旧版本分区未持久化桶数或仅用于 diff 比较，调用方回落表级值。分区从 `listPartitions()` 构造时填入 `PartitionInfo.getBucketCount()`。
2. **`getLogSplit` 增加分区桶数入参**：
   ```java
   int bucketCount = partitionBucketCount != null ? partitionBucketCount : tableInfo.getNumBuckets();
   for (int bucketId = 0; bucketId < bucketCount; bucketId++) { ... }
   ```
   `initLogTablePartitionSplits` 遍历分区时把 `partition.getBucketCount()` 逐分区传入。
3. **非分区表 / 旧分区回退**：`partitionBucketCount == null` 时回落 `tableInfo.getNumBuckets()`，行为与改造前一致。

**改造后效果**：老分区只枚举其真实存在的 bucket、新分区按新桶数枚举，读取不重不漏。

> 湖表读端（`LakeSplitGenerator` 仍按构造注入的表级 `bucketCount` 枚举）不在本期范围：湖表禁止 ALTER `bucket.num`（§2），表级桶数不会被改大，按表级枚举自洽。
...
**服务端 ALTER 与回填**（`AlterBucketNumTest`）：覆盖湖表 ALTER 被拒、非分区表 ALTER 被拒、PK 分区表 ALTER 成功（旧分区保留原桶数、表级值更新为新值、ALTER 后新建分区带新桶数）、Log 分区表 ALTER 成功、回填只影响 `bucket.num.actual` 为空的旧分区（已有桶数的分区跳过，验证幂等）。

**自动分区表缓存刷新**（`AutoPartitionManagerTest`）：验证仅改 `bucket.num` 的 ALTER 触发缓存刷新后，自动预建的新分区采用更新后的表级桶数作为自身 `bucket.num.actual`。

**元数据传播与序列化**：`PartitionRegistrationJsonSerde` 的新旧版本互转（v1 无 `bucket_count` 反序列化为 null、v2 正常回读）；`ClientRpcMessageUtils` 把 `PbPartitionInfo.bucket_count` 正确解析为 `PartitionInfo.getBucketCount()`（缺失映射为 null）。

**客户端端到端**（须补：跨桶数分区的读写正确性 ITCase）：建分区表并写入 → ALTER 放大 `bucket.num` → 再建新分区，覆盖以下断言，才能证明五条读写路径在真实数据上按分区级桶数路由，而非仅靠代码审查：
- 写入：老分区的 append/upsert 仍落在其原桶数的范围内，新分区落在新桶数范围内；
- 点查：老分区、新分区的主键 lookup 都能查回自身写入的数据（验证写路由与 lookup 路由的 N 值一致）；
- 流读：老分区只枚举其真实存在的 bucket、新分区按新桶数枚举，不重不漏；
- 批读与 count(\*) 下推：按各分区实际桶数统计，结果与写入条数一致。

现状：服务端与解析层用例已随实现落地；客户端端到端 ITCase 尚缺，是本期收尾前必须补齐的一项。
